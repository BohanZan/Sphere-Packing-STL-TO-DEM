...
try {
    if (-not $SkipMatlab) {
        ...
        & matlab -batch "addpath(fullfile(pwd,'cpp','benchmarks')); runMatlabBaseline('box',$Repetitions); runMatlabBaseline('refill',$Repetitions); runMatlabBaseline('buddha',$Repetitions); runMatlabBaseline('buddha_gravity',$Repetitions);" -logfile tmp/cpp_matlab_benchmark.log
        if ($LASTEXITCODE -ne 0) { throw 'MATLAB baseline failed.' }
    }
    & python cpp/benchmarks/compare.py --scenario box refill buddha buddha_gravity --repetitions $Repetitions
