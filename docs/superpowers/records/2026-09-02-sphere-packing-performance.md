...
user-owned source plan remains in `docs/superpowers/plans/`.
...
Completed changes include baseline fixtures and benchmarking, cached inward
normals with a disabled-refill guard, batched exact vertical-ray parity,
conservative occupancy triage, batched finite-triangle distance checks,
sphere-neighbour allocation reduction, and preallocated state with incremental
reindexing.
...
incompletes in 11.4865 seconds. A capped four-sphere profile recorded 19,918
calls; `verticalTriangleHit` and `spPointTriangleDistance` recorded zero calls.
