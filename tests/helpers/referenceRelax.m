function state = referenceRelax(context, state, gravity, options, globalPass)
% Historical stepped solver, NOT an oracle for current Algorithm 2/3 motion.
...
d=low;
  ...
  for ix=triangleLo(1):triangleHi(1)
   for iy=triangleLo(2):triangleHi(2)
    for iz=triangleLo(3):triangleHi(3)
     key=spCellKeys([ix iy iz],context.cellKeySpec);
     if iscell(key), key=key{1}; end
     members=spGridLookup(triangleCells,key);
     if ~isempty(members)
      partCount=partCount+1;
      parts{partCount}=members;
     end
    end
   end
  end
 ...
 if ~isequal(pointXY,cachedPointXY)
  ...
  rayKey=spCellKeys(rayIndex,context.xyKeySpec);
  if iscell(rayKey), rayKey=rayKey{1}; end
  rayIds=spGridLookup(context.xyCells,rayKey);
  if isempty(rayIds)
   cachedRayHeights=[];
  else
   rayIds=rayIds(:);
