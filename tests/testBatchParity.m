...
function testNonfiniteQueriesMatchCppRejection(testCase)
context=horizontalContext(0,1e-9);
points=[.2 .2 inf;.2 .2 -inf;.2 .2 NaN;NaN .2 1];
for k=1:4
 verifyError(testCase,@() spExactPointInside(context,points(k,:)),'SpherePacking:InvalidPoint');
end
verifyError(testCase,@() spExactPointInsideBatch(context,points),'SpherePacking:InvalidPoint');
end
