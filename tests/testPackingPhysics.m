...
options.maxCompressionSweeps=Inf;
...
evalc('[assembly,~,~,~,report]=spawnSpheres(model,.2*ones(12,1),12,.2,options);');
...
for k=1:2
 ...
 context=spBuildContext(physicsBoxMesh(shift,shift+4),.25,.5,1e-9);
 state=spEmptyState(context,3);
...
for k=1:numel(rotations)
 ...
 context=spBuildContext(rotated,.25,.5,1e-9);
 state=spEmptyState(context,3);
...
context=spBuildContext(physicsBoxMesh,.25,.25,1e-9);
...
context=spBuildContext(physicsBoxMesh([0 0 0],[20 20 20]),1,.2,1e-9);
...
% A positive b_u requires enough bounded sweeps to reach the far support.
options.maxCompressionSweeps=12;
...
evalc('[a,~,~,~,xReport]=spawnSpheres(physicsBoxMesh([0 0 0],[10 1 1]),[.2;.6],1,1,options);');
verifyEqual(testCase,xReport.acceptedCount,1);
%Five contiguous 2.2-high slabs each exhaust three empty calls.
verifyEqual(testCase,xReport.initialFailures,15);
verifyEqual(testCase,a(1,1),.2,'AbsTol',1e-8);
options.gravity=[0;0;-1];
evalc('[a,~,~,~,zReport]=spawnSpheres(physicsBoxMesh([0 0 0],[10 1 1]),[.2;.6],1,1,options);');
