...
for kind=1:3
    context=spBuildContext(refillMesh(kind),.24,0,1e-9);
    options=smallOptions;
...
[assembly,masses,volume,~,report]=spawnSpheres(boxMesh,[.2*ones(8,1);3],1,0,options);
...
verifyTrue(testCase,contains(output,'sweep 1/2'));
...
options=struct('maxAttempts',4,'maxRefillPasses',3,'gravity',[0 0 -1], ...
    'maxCompressionSweeps',2,'shakeSweeps',1,'compressionTolerance',1e-7);
end
