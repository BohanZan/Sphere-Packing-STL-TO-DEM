...
// I/O IMPLEMENTATION
// Preserve the MATLAB-facing file names, headers, one-based output labels and
// coordinate conventions while using strict stream errors for safe writes.
namespace sp {
namespace {
...
// JSON has no infinity literal. A null statistic explicitly marks a result
// outside double precision, rather than corrupting an otherwise valid report.
void finiteJson(std::ostream &out, double value) {
  if (std::isfinite(value)) out << value;
  else out << "null";
}
void statisticsJson(std::ostream &out, const RadiiStatistics &s) {
  out << "{\"count\":" << s.count << ",\"minimum\":" << s.minimum
      << ",\"maximum\":" << s.maximum << ",\"mean\":" << s.mean
      << ",\"median\":" << s.median << ",\"variance\":";
  finiteJson(out, s.variance);
  out << ",\"standardDeviation\":" << s.standardDeviation << '}';
}
void generationJson(std::ostream &out, const RadiiGenerationReport &g,
                    std::span<const double> accepted) {
  const auto &o = g.options;
  out << "{\"law\":\"index_inverse_power\",\"statisticsConvention\":\"population\""
      << ",\"count\":" << o.count
      << ",\"exponent\":" << g.exponent
      << ",\"targetMedianBeforeClamping\":" << o.median
      << ",\"targetVarianceBeforeClamping\":";
  if (o.variance) out << *o.variance; else out << "null";
  out << ",\"targetStandardDeviationBeforeClamping\":";
  if (o.standardDeviation) out << *o.standardDeviation; else out << "null";
  out << ",\"minimumBound\":" << o.minimum << ",\"maximumBound\":" << o.maximum
      << ",\"clampedBelow\":" << g.clampedBelow
      << ",\"clampedAbove\":" << g.clampedAbove
      << ",\"generationSeconds\":" << g.generationSeconds
      << ",\"beforeClamping\":";
  if (g.beforeClamping) statisticsJson(out, *g.beforeClamping);
  else out << "null";
  out << ",\"afterClamping\":";
  statisticsJson(out, g.afterClamping);
  out << ",\"acceptedStatistics\":";
  if (accepted.empty()) out << "null";
  else statisticsJson(out, radiusStatistics(accepted));
  out << '}';
}
} // namespace

// READSTL Accept both binary STL and strict ASCII solid/facet syntax.
// A binary header may begin with "solid", so the exact byte-size/count check
// decides the format instead of relying on the first characters. Every vertex
// is checked for finiteness and the original facet order is retained.
...
// READNUMBERS Read a flat ordered numeric sequence for prescribed radii or
// replay tapes. Commas are treated as whitespace; malformed tokens fail loudly
// instead of being converted to zero as with atof-style parsing.
...
// WRITEOUTPUTS Persist all established CSV products. Sphere rows stay in
// insertion order, while grid rows use lexical cell labels to retain the
// academic output's stable one-based IDs and corner ordering.
...
// PRINTSUMMARY Report geometry, mass properties, termination status and output
// files in the same human-readable order used by the MATLAB driver.
...
// WRITEREPORTJSON Write machine-readable diagnostics and stage timings. Paths
// and scope names are escaped explicitly so a valid report is produced for
// Windows paths containing quotes or control characters.
void writeReportJson(const std::filesystem::path &path, const Result &result,
                     ...
                     const Random &random, const RadiiGenerationReport *generation) {
  ...
  out << "{\n\"requestedCount\":" << r.requestedCount
      ...
      << ",\n\"initialRelaxationScope\":" << jsonString(r.initialRelaxationScope)
  ...
  out << "],\n\"uniformDraws\":" << random.uniformCount
      ...
      << '\n';
  if (generation) {
    out << ",\"radiusGeneration\":";
    generationJson(out, *generation, result.state.radii);
    out << '\n';
  }
  out << "}\n";
  out.close();
