...
// Cache the immutable STL surface in sparse spatial indices. The public
// queries below deliberately separate conservative candidate generation from
// exact finite-feature distance and ray-parity decisions.
namespace sp {
...
// CELLINDEX Return the clamped zero-based 3-D spatial-hash index of POINT.
// Convert physical coordinates to integer cells without allocating empty
// geometric space; boundary points are deliberately clamped to the last cell.
...
// CLOSESTPOINT Return the closest point on a finite triangle.
// Classify vertex, edge and face Voronoi regions explicitly. Degenerate faces
// fall back to their finite segments so STL defects cannot create a division
// by zero or an infinite supporting-plane contact.
...
// POINTTRIANGLEDISTANCE Measure distance to the finite triangle, including its
// edges and vertices; distance to an infinite plane would permit wall leakage.
...
// TRIANGLECONTACTDISTANCE Find the first swept-sphere contact with one finite
// triangle. Solve the face interval first, then each edge's piecewise segment;
// this covers face, edge and vertex contact along the whole allowed path.
...
// TRIANGLECELLCANDIDATES Raster a conservative finite-feature distance band.
// Small boxes are enumerated directly; long thin triangles use edge/slice
// bounds so a large triangle does not force a full 3-D bounding-box scan.
...
// RAYBOUNDS Expand projected XY bounds for barycentric tolerance and roundoff.
// These bounds only reject impossible downward-ray hits; parity remains exact.
...
// EXACTPOINTINSIDE Test a point by the downward-ray parity rule.
// Projected face candidates are deduplicated by face ID, and equal-height
// crossings retain the strict cutoff used by the MATLAB baseline.
...
// POINTINSIDE Use the occupancy grid only for cells proven wholly inside or
// outside. Margin cells always fall back to exact parity at the queried point.
...
// BUILDCONTEXT Preprocess STL triangles into sparse contact and ray grids.
// Cell spacing covers the largest possible sphere-centre interaction, while
// sparse storage avoids materialising billions of untouched cells.
Context buildContext(Mesh mesh, double maxRadius, double buffer,
  ...
  // Store a length tolerance scaled to the model. The same option therefore
  // behaves consistently for micrometre and metre-sized STL coordinates.
  c.tolerance =
  ...
  // Cell spacing follows the largest sphere diameter plus b_u. This is an
  // indexing invariant: it is not a user-selectable approximation to contact.
  ...
  // Cache finite triangle bands and projected ray coefficients once; placement
  // and relaxation then reuse immutable geometry instead of rebuilding it.
  ...
  // Orient normals by probing just inside/outside the closed surface. The
  // direction is later used to seed refill candidates from active faces.
  ...
  // Optional occupancy labels accelerate obvious interior/exterior cells.
  // Margin cells remain label 2 and are never trusted without exact parity.
  if (opt.occupancyAcceleration) {
    ...
    // Flood-fill 6-connected free components iteratively. A FIFO avoids the
    // recursion depth limit on large connected interiors.
...
// SIGNEDMESHVOLUME Calculate oriented mesh volume from triangle tetrahedra.
// Translate to the first vertex before accumulation to reduce cancellation for
// meshes far from the world origin; the sign is preserved for the caller.
